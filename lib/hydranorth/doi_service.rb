module Hydranorth
  ...
  class DOIService
    ...
    def self.update(generic_file, status_message)
      ezid_identifer = Ezid::Identifier.modify(generic_file.doi, doi_metadata(generic_file, status_message))
      if ezid_identifer.present?
    ...
    private
      ...
      def self.doi_metadata(generic_file, status_message = 'not publicly released')
        return {
          ...
          status: generic_file.private? && generic_file.doi.present? ? "#{Ezid::Status::UNAVAILABLE} | #{status_message}" : Ezid::Status::PUBLIC
        }
      end
  end
end
